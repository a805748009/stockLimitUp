...
def _get_market_data_ex_tuple_period_ori(
    ...
    for stockcode in data_path_dict:
        ...
        data_list = bson.BSON.decode(client.read_local_data(file_name, start_time, end_time)).get('result')
...
def _get_market_data_ex_tuple_period(
    ...
    all_data = _get_market_data_ex_tuple_period_ori(stock_list, period, start_time, end_time)
...
def get_trading_calendar(market, start_time = '', end_time = ''):
    ...
    tdl = client.get_trading_dates_by_market(market, start_time, '', -1)
    ...
    ts = dt.datetime.strptime(start_time, '%Y%m%d') if start_time else tdl[0]
...
def get_trade_times(stockcode):
    ...
    '''
    stockcode_split_pos = stockcode.rfind('.')
    if stockcode_split_pos != -1:
        ins_dl = get_instrument_detail(stockcode)
        product = ins_dl['ProductID']
        stock = stockcode[:stockcode_split_pos]
        market = stockcode[stockcode_split_pos + 1:]
        default = 0
    else:
        market = stockcode
        product = ""
